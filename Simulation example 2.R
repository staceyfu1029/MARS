...
## Functions
simdata <-function(X,t,N,beta,hazard.fun,censor.rate=0.01){
  ...
  survival <- t(apply(XB, 1, FUN=function(x){exp(-cumsum(baseline$hazard*exp(x)))}))
  survival <- cbind(1, survival)
  lifetimes <- apply(survival, 1, FUN=function(x){
    z <- diff(x < runif(1))
    r <- ifelse(all(z==0), t+1, which.max(z))
    return(r)
  })
  data <- data.frame(X)
  data <- dplyr::mutate(data, y = lifetimes)
  cen <- rexp(N,censor.rate)
  data$status <- (data$y<cen) & (data$y<=t)
  data$y <- pmin(data$y,cen)
  data$y[data$y>t] <- t
  return(data)
}

my.hazard <- function(t) {0.01}
...
simulated <- function(n.Study,seed){
  ...
  while(1){
    ...
    N <- sample(80:150,size=n.Study,replace = TRUE)  # Sample size for each individual study
    Dur <- sample(16:24*5,size=n.Study,replace = TRUE)  # Duration for each individual study
    
    data <- data.frame(x=numeric(),y=numeric(),status=logical(),study=integer())
    
    for(i in 1:n.Study){
      p <- runif(1,min=0.45,max=0.55)  # Proportion of MRD negative in each study
      X <- data.frame(x=as.numeric(rbernoulli(n=N[i],p=p))) 
      beta.mat <- data.frame(beta1=1-1/5^((1:Dur[i])/30-1)+beta[i])
      sim <- simdata(X=X,t=Dur[i],N=N[i],beta=beta.mat,hazard.fun = my.hazard,censor.rate = 0.01)
      study <- rep(i,N[i])
      temp <- cbind(sim,study)
      data <- rbind(data,temp)
    }
    
    id <- 1:n.Study
    id.t1 <- sort(sample(1:n.Study,9))
    id.t2 <- sort(c(sample(id.t1,5),sample(id[!(id %in% id.t1)],7)))
    ...
    {
      d <- data[data$study==i,]
      fit <- coxph(Surv(y,status)~x,data = d)
    ...
    {
      d <- data[data$study==i,]
      fit <- survfit(Surv(y,status)~x,data = d)
      time <- sample(8:16*5,1)
...
get_stats <- function(model){
  ...
  for(i in 1:J){
    ...
    w[i] <- integrate(fun2,lower = i-1,upper = i)$value
  }
  HR.mse <- sum(w)/J
  ...
  ## piecewise HR
  b <- c(summ[grep("b1",rownames(summ)),c(1,3,7)])
  ...
  MS.D.hat <- mean(ms0-ms1,na.rm = T)
  MS.D.lower <- quantile(ms0-ms1,probs = 0.025,names = FALSE,na.rm = T)
  MS.D.upper <- quantile(ms0-ms1,probs = 0.975,names = FALSE,na.rm = T)
...
RMST <- function(time,l.0,l.1){
  ...
  RMSTD.hat <- mean(rmst.0-rmst.1)
  RMSTD.lower <- quantile(rmst.0-rmst.1,probs = 0.025,names = FALSE)
  RMSTD.upper <- quantile(rmst.0-rmst.1,probs = 0.975,names = FALSE)
  RMSTR.hat <- median(rmst.0/rmst.1)
  RMSTR.lower <- quantile(rmst.0/rmst.1,probs = 0.025,names = FALSE)
  RMSTR.upper <- quantile(rmst.0/rmst.1,probs = 0.975,names = FALSE)
...
model{
  ...
  Sigma.b[1:J,1:J] <- tau.b * inverse(covar.b[1:J,1:J])
  tau.b <- inverse(sigma.b)
  Sigma.h[1:J,1:J] <- tau.h * inverse(covar.h[1:J,1:J])
...
params <- c("L.0","L.1","beta.0","lambda.adj","beta.adj","lambda.1","b1")
...
rec.out <- foreach(count=1:n.sim, .combine = rbind, 
  ...
  HR.ph.hat <- summ[1,1]
  fun1 <- function(x){
    (1-0.2^((x/3-1))-HR.ph.hat)^2
  }
  HR.ph.se <- summ[1,2]
  HR.ph.lower <- summ[1,3]
  HR.ph.upper <- summ[1,7]
  HR.ph.mse <- integrate(fun1,lower = 0,upper = J)$value/J
...
## 3 methods
c.full <- c(c.hr,c.b,c.sr,c.ms,c.msd,c.rmst,c.rmstd,c.rmstr)
